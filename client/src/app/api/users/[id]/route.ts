...
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = await userService.updateAgent(id, body, currentUser);
    if (!result.success) {
      let status = 400;
    ...
    // Invalidate cached team list on details update
...
export async function DELETE(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const result = await userService.deleteAgent(id, currentUser);
    if (!result.success) {
      let status = 400;
      if (result.message.includes('Forbidden')) {
        status = 403;
    ...
    // Invalidate cached team list on agent deletion
    await invalidateTenantTeamCache(tenantId);
